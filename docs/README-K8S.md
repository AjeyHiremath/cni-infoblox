...
**CNI Infoblox client:**
  This is the component that interfaces with Infoblox to perform the IPAM functions. This is typically deployed as a kubernetes daemonset (cni-infoblox-client) on each node.
...
You have to update this base64 encoded value in k8s/cni-infoblox-client.yaml in below mentioned section.
...
A network automatically allocated by CNI Infoblox client if subnet is not metioned.
...
```
    kubectl create -f k8s/cni-infoblox-client.yaml
```
cni-infoblox-client daemonset should be created before starting the plugin. It requied a docker image which is available at [infoblox/cni-infoblox-client](https://hub.docker.com/r/infoblox/cni-infoblox-client/) which has the binary in an image of cni-infoblox-client.

NOTE: Don't forget to update base64 encoded wapi-password in k8s/cni-infoblox-client.yaml
